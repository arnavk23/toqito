...
def state_exclusion(
    ...
    **kwargs: Any,
    ...
    When `measurement = "ppt"`, the measurement operators are additionally restricted to be PPT (positive
    partial transpose) across the bipartition described by `subsystems` and `dimensions`. This yields a bound in
    the relaxation hierarchy \(\text{SEP} \subseteq \text{PPT} \subseteq \text{global}\). Because exclusion
    *minimizes* the overlap, the PPT measurement set being a restriction of the global one implies

    \[
        \text{global exclusion} \;\leq\; \text{PPT exclusion} \;\leq\; \text{separable exclusion}.
    \]

    The min-error PPT exclusion primal mirrors the standard exclusion primal with the extra PPT constraints,
        ...
        \begin{equation}
            \begin{aligned}
                ...
                \text{subject to:} \quad & \sum_{i=1}^n M_i = \mathbb{I}_{\mathcal{X}}, \\
                                            ...
                                            & \Gamma_j(M_i) \succeq 0 \quad \forall\, i, j,
            \end{aligned}
        \end{equation}
    \]

    where \(\Gamma_j\) denotes the partial transpose on subsystem \(j\). Its dual, derived by SDP duality
    (note the PPT dual variable \(\Gamma(Q_i)\) sits on the side opposite to the distinguishability case
    because exclusion minimizes rather than maximizes), reads

    \[
        \begin{equation}
            \begin{aligned}
                \text{maximize:} \quad & \text{Tr}(Y) \\
                \text{subject to:} \quad & p_i \rho_i - Y \succeq \Gamma(Q_i) \quad \forall\, i, \\
                                            & Q_i \succeq 0, \quad Y \in \text{Herm}(\mathcal{X}).
            \end{aligned}
        \end{equation}
    \]

    For the unambiguous PPT strategy, the PPT constraint is also imposed on the inconclusive operator
    \(\mathbb{I} - \sum_i M_i\), and the dual gains an extra positive-semidefinite variable \(R\) (with
    \(\Gamma(R)\)) corresponding to that constraint.

    **Notation.** In the formulation above, \(\Gamma_j\) is the partial transpose on subsystem \(j\)
    (`picos.partial_transpose` over `subsystems`/`dimensions`); \(M_i\) are the measurement operators
    (`measurements`); \(Y\) is the dual Hermitian variable; \(Q_i\) (and \(R\) for the unambiguous case)
    are the positive-semidefinite dual variables generated by the PPT constraints.
    ...
    Examples:
        ...
        PPT state exclusion. For the two-qubit ensemble below the global exclusion value is strictly smaller
        than the PPT exclusion value, certifying that no PPT (and hence no separable/LOCC) measurement can match
        the global optimum.
        ...
        vecs = [(0, 1, 1, 1), (0, 0, 1, 1), (1, 0, 1, 1)]
        kets = [np.array(v, dtype=complex).reshape(-1, 1) / np.linalg.norm(v) for v in vecs]
        states = [to_density_matrix(k) for k in kets]

        global_val, _ = state_exclusion(states, primal_dual="dual", cvxopt_kktsolver="ldl")
        ppt_val, _ = state_exclusion(
            states, measurement="ppt", subsystems=[0], dimensions=[2, 2], primal_dual="dual", cvxopt_kktsolver="ldl"
        )

        print(np.around([global_val, ppt_val], decimals=3))
        ```
    ...
    if measurement == "ppt":
        ...
        if int(np.prod(dimensions)) != dim:
            raise ValueError("The product of 'dimensions' must equal the dimension of the input states.")
...
def _ppt_primal(
    ...
    **kwargs,
    ...
    """Solve the primal problem for quantum state exclusion with PPT measurements.

    The exclusion SDP *minimizes* the overlap. PPT constraints are imposed on every measurement
    operator (including the inconclusive operator for the unambiguous strategy).
    """
    n = len(vectors)
    problem = picos.Problem()
    measurements = [picos.HermitianVariable(f"M[{i}]", (dim, dim)) for i in range(n)]
    ...
    unnormalized_dms = [p * rho for (p, rho) in zip(probs, dms)]
    sum_of_unnormalized_dms = picos.sum(unnormalized_dms)
    problem.add_list_of_constraints(m | rho == 0 for (m, rho) in zip(measurements, unnormalized_dms))
...
def _ppt_dual(
    ...
    **kwargs,
    ...
    float,
    list[picos.HermitianVariable] | tuple[picos.HermitianVariable, picos.HermitianVariable, picos.RealVariable],
]:
    r"""Solve the dual problem for quantum state exclusion with PPT measurements.

    Because the exclusion SDP minimizes the overlap (whereas distinguishability maximizes it), the dual
    differs structurally from the distinguishability PPT dual: each PPT measurement constraint contributes a
    partial-transposed dual variable :math:`\Gamma(Q_i)` on the side opposite to the distinguishability case.
    """
    n = len(vectors)
    dms = [to_density_matrix(vector) for vector in vectors]
    problem = picos.Problem()
    ...
    if strategy == "min_error":
        y_var = picos.HermitianVariable("Y", (dim, dim))
        q_vars = [picos.HermitianVariable(f"Q[{i}]", (dim, dim)) for i in range(n)]

        problem.add_list_of_constraints([q_var >> 0 for q_var in q_vars])
        problem.add_list_of_constraints(
            [
                probs[i] * dms[i] - y_var
                >> picos.partial_transpose(q_vars[i], subsystems=subsystems, dimensions=dimensions)
                for i in range(n)
            ]
        )
        ...
        measurements = [problem.get_constraint(n + k).dual for k in range(n)]
        return solution.value, measurements

    # Unambiguous strategy.
    lagrangian_variable_big_n = picos.HermitianVariable("N", (dim, dim))
    lagrangian_variable_big_r = picos.HermitianVariable("R", (dim, dim))
    q_vars = [picos.HermitianVariable(f"Q[{i}]", (dim, dim)) for i in range(n)]
    lagrangian_variables_a = picos.RealVariable("a", n)

    problem.add_constraint(lagrangian_variable_big_n >> 0)
    problem.add_constraint(lagrangian_variable_big_r >> 0)
    problem.add_list_of_constraints([q_var >> 0 for q_var in q_vars])

    unnormalized_dms = [p * rho for (p, rho) in zip(probs, dms)]
    sum_of_unnormalized_dms = picos.sum(unnormalized_dms)
    transposed_big_r = picos.partial_transpose(lagrangian_variable_big_r, subsystems=subsystems, dimensions=dimensions)

    problem.add_list_of_constraints(
        (
            lagrangian_variable_big_n
            + lagrangian_variables_a[i] * unnormalized_dms[i]
            + transposed_big_r
            - picos.partial_transpose(q_vars[i], subsystems=subsystems, dimensions=dimensions)
            >> sum_of_unnormalized_dms
        )
        for i in range(n)
    )

    problem.set_objective("max", 1 - picos.trace(lagrangian_variable_big_n) - picos.trace(lagrangian_variable_big_r))
    solution = problem.solve(solver=solver, primals=None, **kwargs)

    return solution.value, (lagrangian_variable_big_n, lagrangian_variable_big_r, lagrangian_variables_a)
